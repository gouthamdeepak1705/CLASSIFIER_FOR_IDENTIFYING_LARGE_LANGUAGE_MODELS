...
def generate_until_full_stop_bloom(truncated_sentence, model, tokenizer, params=hyperparameters, min_word_count=10, max_retries=20):
...
file_path = "2000.xlsx" 
output_file_path = "EnglishTenseWithBLOOMCompletions_GPU_Progress.xlsx"
temp_save_path = "BLOOM_checkpoint.xlsx"
...
if os.path.exists(temp_save_path):
    ...
    df_checkpoint = pd.DataFrame(columns=['Truncated_Sentence', 'BLOOM_xj', 'BLOOM_complete'])
    completed_indexes = []
...
for batch_num, i in enumerate(range(0, len(df), batch_size)):
    ...
    for sentence in batch:
        completion = generate_until_full_stop_bloom(sentence, model, tokenizer, min_word_count=10)
        completion_list.append(completion)
        combined_list.append(f"{sentence} {completion}")  

    df_temp = pd.DataFrame({'Truncated_Sentence': batch,
                            'BLOOM_xj': completion_list[-batch_size:],
                            'BLOOM_complete': combined_list[-batch_size:]})
